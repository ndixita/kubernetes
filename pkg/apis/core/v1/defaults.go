...
func defaultPodResources(obj *v1.Pod) {
	if obj.Spec.Resources.Limits == nil || len(obj.Spec.Resources.Limits) == 0 {
		return
	}

	var reqs v1.ResourceList
	reqs = obj.Spec.Resources.Requests
	...
	for key, value := range podReqsFromContainers {
		if _, exists := reqs[key]; !exists && resourcehelper.IsSupportedPodLevelResource(key) {
	...
	if len(reqs) > 0 {
		obj.Spec.Resources.Requests = reqs
	}
}
