...
func remainingPodMemReqPerContainer(pod *v1.Pod) int64 {
	...
	if pod.Spec.Resources.Requests.Memory() == nil || pod.Spec.Resources.Requests.Memory().Value() == 0 {
	...
	totalContainersReq := resourcehelper.PodRequests(pod, resourcehelper.PodResourcesOptions{
		InPlacePodVerticalScalingEnabled: true,
		UseContainerLevelResources:       true,
		ExcludeOverhead:                  true,
	})

	remainingMemory = pod.Spec.Resources.Requests.Memory().Value() - totalContainersReq.Memory().Value()
