...
func NewStateCheckpoint(stateDir, checkpointName string) (State, error) {
	...
	if err != nil {
		return nil, fmt.Errorf("failed to initialize checkpoint manager for pod resource information tracking: %v", err)
	}
...
func restoreState(checkpointManager checkpointmanager.CheckpointManager, checkpointName string) (PodResourceInfoMap, checksum.Checksum, error) {
	...
	if err != nil {
		return nil, 0, fmt.Errorf("failed to get pod resource info: %w", err)
	}
...
func (sc *stateCheckpoint) storeState() error {
	resInfo := sc.cache.GetPodResourceInfoMap()
...
func (sc *stateCheckpoint) SetContainerResources(podUID types.UID, containerName string, resInfo v1.ResourceRequirements) error {
	sc.mux.Lock()
	defer sc.mux.Unlock()
	err := sc.cache.SetContainerResources(podUID, containerName, resInfo)
	if err != nil {
		return err
	}
...
// SetPodResourceInfoMap sets pod resource information
func (sc *stateCheckpoint) SetPodResourceInfoMap(podUID types.UID, resInfo PodResourceInfo) error {
	sc.mux.Lock()
	defer sc.mux.Unlock()
	err := sc.cache.SetPodResourceInfoMap(podUID, resInfo)
	if err != nil {
		return err
	}
	return sc.storeState()
...
func (sc *noopStateCheckpoint) SetPodResourceInfoMap(_ types.UID, _ PodResourceInfo) error {
	return nil
}
