...
// SetContainerResoruces sets resources information for a pod's resources at pod-level.
...
func (sc *stateCheckpoint) SetPodResourceInfoMap(podUID types.UID, resourceInfo PodResourceInfo) error {
	sc.mux.Lock()
	defer sc.mux.Unlock()
	err := sc.cache.SetPodResourceInfoMap(podUID, resourceInfo)
	if err != nil {
		return err
	}
	return sc.storeState()
