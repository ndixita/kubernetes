...
func (s *stateMemory) GetContainerResources(podUID types.UID, containerName string) (v1.ResourceRequirements, bool) {
	...
	resources, ok := s.podResources[podUID].ContainerResources[containerName]
...
func (s *stateMemory) SetContainerResources(podUID types.UID, containerName string, resources v1.ResourceRequirements) error {
	...
	klog.V(3).InfoS("Updated container resource info", "podUID", podUID, "containerName", containerName, "resources", resources)
	return nil
}
...
func (s *stateMemory) SetPodLevelResources(podUID types.UID, resources v1.ResourceRequirements) error {
	...
	klog.V(3).InfoS("Updated pod resource info", "podUID", podUID, "resources", resources)
	return nil
}

func (s *stateMemory) SetPodResourceInfoMap(podUID types.UID, resources PodResourceInfo) error {
	s.Lock()
	defer s.Unlock()

	s.podResources[podUID] = resources
	klog.V(3).InfoS("Updated pod resource information", "podUID", podUID, "information", resources)
	return nil
}
