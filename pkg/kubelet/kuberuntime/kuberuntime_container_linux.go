...
func getCPULimit(pod *v1.Pod, container *v1.Container) *resource.Quantity {
	if resourcehelper.IsPodLevelResourcesSet(pod, utilfeature.DefaultFeatureGate.Enabled(kubefeatures.PodLevelResources)) {
		...
		// to be set.
		if container.Resources.Limits.Cpu().IsZero() && !pod.Spec.Resources.Limits.Cpu().IsZero() {
...
func getMemoryLimit(pod *v1.Pod, container *v1.Container) *resource.Quantity {
	if resourcehelper.IsPodLevelResourcesSet(pod, utilfeature.DefaultFeatureGate.Enabled(kubefeatures.PodLevelResources)) {
		...
		// limit is used in the calculation for components relying on linux resource limits
		// to be set.
		if container.Resources.Limits.Memory().IsZero() && !pod.Spec.Resources.Limits.Memory().IsZero() {
